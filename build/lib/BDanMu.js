function BDanMu() {
    ...
    this.danMuDoms = [];
    this.video_el = {
        "tup.yinghuacd.com": [".dplayer-video-wrap"]
    }
    this.currentSite = "";
}

BDanMu.prototype.init = function (site) {
    if (!this.initFlag) {
        this.currentSite = location.hostname;
        if (site !== null && site !== undefined && this.currentSite === "localhost") {
            this.currentSite = site;
        }
        this.pDom = this.video_el[this.currentSite];
        let p = $(this.pDom)
        if (p.length < 1) {
